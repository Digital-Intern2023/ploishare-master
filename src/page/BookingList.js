...
function BookingList() {
  ...
  const fetchData = async () => {
    originData.length = 0;
    await axios
      .get(API_URL + "api/getbookingbyemail/" + email)
      .then((response) => {
        ...
        response.data.map((item, index) => {
          ...
          originData.push({
            ...
            endDateTime: new Date(item.startDateTime).toLocaleString(
              "th",
              options
            ),
  ...
  const updateStatus = async (id, status, license, startMile) => {
    ...
    try {
      await axios
        .get(API_URL + "api/getcarbyid/" + license)
        .then((response) => {
          ...
          if (status === 2) {
            Swal.fire({
              ...
              preConfirm: (startMile) => {
                axios
                  .post(API_URL + "api/updatebookingstartmile", {
                    id: id,
                    startMile: startMile,
                  })
                  .then((response) => {
                    console.log(response.data);
                    axios
                      .post(API_URL + "api/updatebookingstatus", {
                        id: id,
                        status: status,
                      })
                      .then((response) => {
            ...
            Swal.fire({
              ...
              preConfirm: (endMile) => {
                if (endMile < startMile) {
                  ...
                  axios
                    .post(API_URL + "api/updatebookingendmile", {
                      id: id,
    ...
    try {
      Swal.fire({
        ...
        preConfirm: (image) => {
          ...
          axios
            .post(API_URL + "api/upload-file", formData, {
              headers: {
            ...
            .then((response) => {
              console.log(response.data);
              axios
                .post(API_URL + "api/updatebookingimage", {
                  id: id,
                  image: response.data,
                })
                .then((response) => {
    ...
    <div>
      ...
      <div>
        ...
        >
          ...
          >
            {" "}
            <InfoCircleOutlined twoToneColor="#ffffff" />
          </strong>
        </div>
        <Modal
        ...
        >
          <ul>
            <il style={{ fontFamily: "Noto Sans Thai" }} className="mr-4">
              {" "}
              <strong>
                <p className="wait-lightdot mr-2" />
                รออนุมัติ คือ สถานะรออนุมัติคำสั่งจองรถจากผู้ดูแลระบบ
              </strong>
              <p>
                ทางผู้ดูแลระบบจะทำการตรวจสอบข้อมูล
                และอนุมัติคำสั่งจองของท่านให้ไวที่สุด
              </p>
            </il>{" "}
            <br />
            <il style={{ fontFamily: "Noto Sans Thai" }} className="mr-4">
              {" "}
              <strong>
                <p className="turn-on-lightdot mr-2" />
                เปิดใช้งาน คือ สถานะรออนุมัติคำสั่งจองรถจากผู้ดูแลระบบ
              </strong>
              <p>
                เมื่อเปิดการใช้งานรถ
                ระบบจะแจ้งเตือนให้กรอกเลขไมล์รถยนต์ปัจจุบันก่อนใช้งานรถยนต์
              </p>
            </il>{" "}
            <br />
            <il style={{ fontFamily: "Noto Sans Thai" }} className="mr-4">
              {" "}
              <strong>
                <p className="turn-off-lightdot mr-2" />
                ปิดใช้งาน คือ สถานะรออนุมัติคำสั่งจองรถจากผู้ดูแลระบบ
              </strong>
              <p>
                ทางผู้ดูแลระบบจะทำการตรวจสอบข้อมูล
                และอนุมัติคำสั่งจองของท่านให้ไวที่สุด
            ...
            <il style={{ fontFamily: "Noto Sans Thai" }} className="mr-4">
              {" "}
              <strong>
                <p className="success-lightdot mr-2" />
                เสร็จสิ้น คือ สถานะรออนุมัติคำสั่งจองรถจากผู้ดูแลระบบ
              </strong>
              <p>
                ทางผู้ดูแลระบบจะทำการตรวจสอบข้อมูล
                และอนุมัติคำสั่งจองของท่านให้ไวที่สุด
            ...
            <il style={{ fontFamily: "Noto Sans Thai" }} className="mr-4">
              ...
              <p>
                *
                เมื่อเปิดใช้งานรถยนต์ทางผู้ยืมจะต้องเข้ามาปิดการใช้งานบนเว็บไซต์
              </p>
            </il>{" "}
            <br />
            <il style={{ fontFamily: "Noto Sans Thai" }} className="mr-4">
              ...
              <p>
                **
                ทางเรามีบัตรให้บริการเติมน้ำมันโดยผู้จองไม่จำเป็นต้องจ่ายค่าน้ำมันขณะใช้งานรถ
                ทางระบบจะเรียกเก็บเงินภายหลัง โดยคำนวณค่าใช้จ่ายจากเลขไมล์รถยนต์
              </p>
            </il>{" "}
            <br />
          </ul>
              ...
              <div className="item-list" key={index}>
                <Card className="min-card">
                  <Card.Header style={{ textAlign: "center" }}>
                    {item.status === "0" ? (
                      <p className="inline-block">
                        <div className="wait-lightdot mr-1" />
                        รออนุมัติ{" "}
                        <div className="absolute top-3 right-3 cursor-pointer">
                          <FcCancel
                            size={25}
                            onClick={() => {
                              updateCancel(item.id);
                            }}
                          />
                        </div>
                      </p>
                      ...
                      <p className="inline-block">
                        <div className="allowed-lightdot mr-1" />
                        อนุมัติแล้ว{" "}
                        <div className="absolute top-3 right-3 cursor-pointer">
                          <FcCancel
                            size={25}
                      ...
                      <p className="inline-block">
                        <div className="success-lightdot mr-1" />
                        เสร็จสิ้น
                      </p>
                    ) : (
                      <p className="inline-block">
                  ...
                  <Card.Body style={{ minHeight: "180px" }}>
                    ...
                    <Card.Text>
                      ใช้งานวันที่ : {item.startDateTime}
                      <br />
                    </Card.Text>
                    {item.status === "2" ? (
                      <Card.Text>ไมล์เริ่มต้น : {item.startMile}</Card.Text>
                    ) : item.status === "3" ? (
                      <Card.Text>
                        ไมล์เริ่มต้น : {item.startMile}
                        <br />
                      ...
                      <div
                        className={
                          item.status === "0"
                            ? "btn wait-cardlist"
                            : item.status === "1"
                            ? "btn turn-on-cardlist"
                            : item.status === "2"
                            ? "btn turn-off-cardlist"
                            : "btn success-cardlist"
                        }
                        onClick={
                          item.status === "0"
                            ? () => {
