...
const ContentBooking = () => {
        ...
        originData.push({
          ...
          startDateTime: new Date(
            Date.UTC(
              ...
              new Date(item.startDateTime).getUTCSeconds()
            )
          ).toLocaleString("th", options),
          endDateTime: new Date(
            Date.UTC(
              ...
              new Date(item.endDateTime).getUTCSeconds()
            )
          ).toLocaleString("th", options),
          bookingDate: new Date(
            Date.UTC(
              ...
              new Date(item.bookingDate).getUTCSeconds()
            )
          ).toLocaleString("th", options),
  ...
  const searchDate = () => {
    ...
    if (startDate === "" || endDate === "") {
      ...
      axios
        .post(API_URL + "api/searchbookingbydaterange", {
  ...
  const searchEmail = (value) => {
    ...
    if (value === "") {
      ...
      axios
        .post(API_URL + "api/searchbookingbyemail", { uEmail: value })
        .then((response) => {
          console.log(response.data);
          originData.length = 0;
          const new_data = [];
  ...
  const deleteBooking = (record) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(API_URL + "api/deletebooking/" + record.id)
          .then((response) => {
            console.log(response.data);
            axios
              .delete(API_URL + "api/delete-files-url", {
                data: { url: record.image },
              })
              .then((response) => {
                console.log(response.data);
              });
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      render: (image) => (
        <Image width={75} height={75} src={image !== null ? image : NO_Img} />
      ),
      width: "8%",
      editable: true,
    },
        ...
        return editable ? (
          ...
          <div>
            {record.status === "0" ? (
              ...
              <div className="text-blue-700 font-bold">ยืนยันแล้ว</div>
            ) : record.status === "2" ? (
              ...
              <div className="text-green-700 font-bold">เสร็จสิ้น</div>
            ) : (
              <div className="text-red-700 font-bold">ยกเลิก</div>
            )}
          </div>
        );
      },
