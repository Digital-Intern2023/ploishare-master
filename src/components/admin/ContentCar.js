...
function ContentCar() {
  ...
  const deleteCar = (record) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(API_URL + "api/deletecar/" + record.license)
          .then((response) => {
  ...
  const updateCarStatus = (record) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(API_URL + "api/updatecarstatus", {
      ...
      render: (image) => {
        ...
        return (
          <Image.PreviewGroup>
            {images.map((item) => {
              console.log(item);
              return <Image width={75} height={75} src={item} />;
            })}
          </Image.PreviewGroup>
        );
      },
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return editable ? (
          ...
          <button
            className={
              record.status === 1 ? "btn btn-success" : "btn btn-danger"
            }
        ...
        <Layout style={{ padding: "0 24px 24px", margin: "24px 0" }}>
          ...
          >
            <div style={{ textAlign: "right", paddingBottom: 20 }}>
              <Link to={"/addcar"} className="btn-add">
                {" "}
                + Add
              </Link>
            </div>
